    ...
    with open("output.txt", 'w') as file:
        while (temperature > stopping_temperature) and (iteration < max_iterations):
            ...
            for neighbor in neighbors:
                ...
                if (((current_cost < best_cost) and (unsafe_planes <= unsafe_planes_sol) and (num_of_crashes <= num_of_crashes_sol)) or ((unsafe_planes < unsafe_planes_sol) and (num_of_crashes <= num_of_crashes_sol)) or (num_of_crashes < num_of_crashes_sol))  or math.exp(-delta_cost / temperature) > random.random(): 
